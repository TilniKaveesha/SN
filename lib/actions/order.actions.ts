...
export async function updateOrderToPaid(orderId: string) {
  try {
    ...
    await order.save()
    if (!process.env.MONGODB_URI?.startsWith("mongodb://localhost")) await updateProductStock(order._id.toString())
...
//payway
export async function createPayWayOrder(orderId: string) {
  await connectToDatabase()
  try {
...
export async function verifyPayWayPayment(orderId: string, tran_id: string) {
  ...
  try {
    ...
    if (paymentData.status === "0") {
      ...
      return {
        success: true,
        message: "Your order has been successfully paid via PayWay",
      }
    } else {
